...
namespace Server
{
    public class FTPserver
    {
        ...
        public void FTPUploadFile(string filename, string sender, string receiver)
        {
            ...
            RenameFile(filename, sender, receiver);
            Console.ReadKey();
        }
        ...
        public void FTPDownloadFile(string filename)
        {
            ...
            do
            {
                if (!wc.IsBusy)
                {
                    ...
                    wc.DownloadFileAsync(new Uri(@UriFTP + Path.GetFileName(filename)), @"D:/" + Path.GetFileName(filename));
                    flag = false;
                }
            } while (flag);
            Console.ReadKey();
